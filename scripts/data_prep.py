...
def actransform(data_tek, data_wo, data_mesin, data_shift, data_nwo):
    ...
    # pengecualian untuk day-on atau libur: ['5 Januari 2019', '3 April 2019', '13 April 2019']
    for idx in act.index:
    ...
    # info shift
    data_shift = data_shift[['name', 'year', 'week', 'day', 'day_cat', 'shift']].copy()
    ...
    # hapus nama teknisi diluar utility
    act = act.drop(act[act.name == 'Agus Wildan Hilmi'].index, axis=0)
    ...
    act = act.drop(act[act.name == 'Ferry Maulana'].index, axis=0)
    ...
    # re-sort column
    act = act[['name', 'year', 'month', 'week', 'day', 'day_cat', 'shift', 'task_cat', 'wo', 'wo_status', 'system', 'machine', 'date_start', 'date_finish', 'duration']]
    ...
    act = act.sort_values(['name', 'date_start'], ascending=True)
    ...
    # act.loc[:, 'year'] = act.loc[:, 'year'].fillna(2019) # sementara
    act.loc[:, 'year'] = act['year'].astype('int64')
    # act.loc[:, 'month'] = act.loc[:, 'month'].fillna(5) # sementara
    ...
    # sort dataframe
    act = act.sort_values(['name', 'date_start'])
    act.reset_index(inplace=True, drop=True)
...
def prep4chart(df, rpmtdata, rpairdata):
    ...
    group1 = df.groupby(['year', 'month']).apply(lambda x: pd.Series({'mh_total': x[(x['task_cat'] != 'Break') & (x['task_cat'] != 'Planned')]['duration'].sum()})).reset_index()
    group2 = df.groupby(['year', 'month']).apply(lambda x: pd.Series({'mh_real': x[(x['task_cat'] != 'Break') & (x['task_cat'] != 'Planned') & (x['task_cat'] != 'Unplanned')]['duration'].sum()})).reset_index()
    mergroup = group1.merge(group2, on=['year', 'month'])
    mergroup['%_mhreal'] = mergroup['mh_real'] / mergroup['mh_total'] * 100.0
    
    # wo pmt dataframe preparation
    realpmt = rpmtdata.loc[rpmtdata['nomor_wo_pmt'].notnull(), ['nama_detail', 'nomor_wo_pmt', 'status_wo', 'tanggal_rencana', 'tanggal_selesai']]
    realpmt.columns = ['machine', 'wo', 'wo_status', 'date', 'date_finish']
    ...
    realpmt = realpmt[['machine', 'year', 'year_help', 'month', 'month_help', 'week', 'week_help', 'wo', 'wo_status', 'date', 'date_finish']]
    ...
    group3 = realpmt.groupby(['year', 'month']).apply(lambda x: pd.Series({'wo_total': x['wo'].nunique()})).reset_index()
    group4 = realpmt.groupby(['year_help', 'month_help']).apply(lambda x: pd.Series({'wo_rlz': x[(x['wo_status'] == 'done') & (x['week'] == x['week_help'])]['wo'].nunique()})).reset_index()
    group4.columns = ['year', 'month', 'wo_rlz']
    rlzpmt = group3.merge(group4, on=['year', 'month'])
    ...
    realrep = rpairdata.loc[rpairdata['status_wo'] != 'rejected', ['nama_detail', 'nomor_wo_repair', 'status_wo', 'waktu_pengajuan_servis', 'waktu_selesai']]
    realrep.columns = ['machine', 'wo', 'wo_status', 'date', 'date_finish']
    ...
    realrep = realrep[['machine', 'year', 'year_help', 'month', 'month_help', 'week', 'week_help', 'wo', 'wo_status', 'date', 'date_finish']]
    ...
    group5 = realrep.groupby(['year', 'month']).apply(lambda x: pd.Series({'wo_total': x['wo'].nunique()})).reset_index()
    group6 = realrep.groupby(['year_help', 'month_help']).apply(lambda x: pd.Series({'wo_rlz': x[x['wo_status'] == 'done']['wo'].nunique()})).reset_index()
    group6.columns = ['year', 'month', 'wo_rlz']
    datareprlz = group5.merge(group6, on=['year', 'month'])
    datareprlz['year'] = datareprlz['year'].astype('int64')
    datareprlz['month'] = datareprlz['month'].astype('int64')
    
    notrlz = 0
    for i in datareprlz.index:
        if i == 0:
